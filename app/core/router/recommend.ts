...
function parseMethod(value: unknown): Parsed<MethodCapability> {
  ...
  const uniqueModalities = parsedModalities.ok ? uniqueStrings(parsedModalities.value, 'modalities') : parsedModalities
  const uniqueRequiredPriors = requiredPriors.ok ? uniqueStrings(requiredPriors.value, 'requiredPriors') : requiredPriors
  const uniqueSupportedGoals = supportedGoals.ok ? uniqueStrings(supportedGoals.value, 'supportedGoals') : supportedGoals
  const canonicalOutputs = parsedOutputs.ok && uniqueOutputs.ok && parsedOutputs.value.every((output, index) => output === outputs.filter((known) => parsedOutputs.value.includes(known))[index])
  if (!identifier(parsed.value.id) || !aliases.ok || !nonempty(parsed.value.version) || !parsedModalities.ok || !uniqueModalities.ok || !inSet(parsed.value.maxScale, scales.slice(0, 4)) || !parsedOutputs.ok || !uniqueOutputs.ok || !canonicalOutputs || !requiredPriors.ok || !uniqueRequiredPriors.ok || !supportedGoals.ok || !uniqueSupportedGoals.ok || ![1, 2, 3].includes(parsed.value.resourceTier as number) || !nonempty(parsed.value.installCommand) || !nonempty(parsed.value.license) || !absoluteHttpUrl(parsed.value.sourceUrl) || !absoluteHttpUrl(parsed.value.docsUrl) || !absoluteHttpUrl(parsed.value.paperUrl) || typeof parsed.value.executable !== 'boolean') return failure('method has invalid fields')
