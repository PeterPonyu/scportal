...
export function validateExecutableConfig(value: unknown): ExecutableConfig {
  ...
  for (const [key, parameter] of Object.entries(parameters)) {
    ...
    if (typeof parameter !== 'string' && typeof parameter !== 'boolean' && (typeof parameter !== 'number' || !Number.isFinite(parameter))) throw new Error(`executable config.parameters.${key} must be finite JSON data`)
  ...
  for (const key of ['graph', 'pseudotime', 'branch'] as const) if (output[key] !== undefined) outputs[key] = nonblank(output[key], `executable config.outputs.${key}`)
  ...
  if (!Number.isInteger(provenance.recommendationSeed) || (provenance.recommendationSeed as number) < 0 || (provenance.recommendationSeed as number) > 0xffffffff || outcome.status !== 'OK' || nonblank(outcome.methodId, 'executable config.provenance.outcome.methodId') !== nonblank(method.id, 'executable config.method.id') || !/^[a-f0-9]{64}$/.test(nonblank(provenance.profileFingerprint, 'executable config.provenance.profileFingerprint'))) throw new Error('executable config provenance schema validation failed')
  return deepFreeze({ schemaVersion: '1.0', routerVersion: nonblank(config.routerVersion, 'executable config.routerVersion'), evidenceVersion: nonblank(config.evidenceVersion, 'executable config.evidenceVersion'), method: { id: method.id as string, version: nonblank(method.version, 'executable config.method.version'), install }, preprocessing: { modality: preprocessing.modality as ExecutableConfig['preprocessing']['modality'], normalization: nonblank(preprocessing.normalization, 'executable config.preprocessing.normalization'), featureSelection: nonblank(preprocessing.featureSelection, 'executable config.preprocessing.featureSelection') }, parameters: parsedParameters, outputs, downstream, provenance: { recommendationSeed: provenance.recommendationSeed as number, methodSource: nonblank(provenance.methodSource, 'executable config.provenance.methodSource'), generatedAt: nonblank(provenance.generatedAt, 'executable config.provenance.generatedAt'), profileFingerprint: provenance.profileFingerprint as string, outcome: { status: 'OK', methodId: method.id as string } } })
}
