...
describe('author catalog identity', () => {
  ...
  it('keeps reserved GEO identities correctly labeled and empty of UCB/sleep scores', async () => {
    ...
    assert.equal(byId.s_subs8_cd34.studyGroup, 'cd34_hspc')
    assert.equal(byId.gse132188_endo.studyGroup, 'pancreas_endo')
    assert.equal(byId.gse117498_pheno_hsc.studyGroup, 'pheno_hsc')
    assert.notEqual(byId.s_subs8_cd34.studyGroup, 'radiation_lsk')
    assert.notEqual(byId.gse132188_endo.studyGroup, 'radiation_lsk')
  })
})
